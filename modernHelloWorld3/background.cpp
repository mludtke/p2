...
{
	vec4 lighter_color(this->MakeColor(80, 20, 0));
	vec4 darker_color = vec4(vec3(lighter_color) / 8.0f, 1.0f);
	this->colors[0] = darker_color;
	this->colors[1] = lighter_color * vec4(3.0f, 1.0f, 1.0f, 1.0f);
	this->colors[2] = lighter_color * vec4(3.0f, 1.0f, 1.0f, 1.0f);
...
bool Background::Initialize()
{
	...
	this->vertices.push_back(VertexAttributesP(vec3(0.5f, 0.5f, 0.0f)));

	this->vertex_indices.push_back(0);
	this->vertex_indices.push_back(4);
...
void Background::Draw(const mat4 & projection, mat4 modelview, const ivec2 & size, GLfloat xRot, GLfloat yRot, const float time)
{
	assert(false);
}
